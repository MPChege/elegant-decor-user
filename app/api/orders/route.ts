...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!supabaseUrl || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      console.error('[Orders API] ❌ Supabase environment variables are not set!')
    ...
    if (supabaseUrl === 'https://placeholder.supabase.co' || supabaseUrl.includes('localhost') || supabaseUrl.includes('127.0.0.1')) {
      console.error('[Orders API] ❌ NEXT_PUBLIC_SUPABASE_URL is not a production URL! Current value:', supabaseUrl)
      return NextResponse.json(
        { 
          success: false, 
          error: 'Server configuration error. Please contact support.' 
    ...
    if (!supabaseUrl.startsWith('https://') || !supabaseUrl.includes('.supabase.co')) {
      console.error('[Orders API] ❌ NEXT_PUBLIC_SUPABASE_URL is not a valid Supabase URL:', supabaseUrl)
      return NextResponse.json(
        { 
          success: false, 
          error: 'Server configuration error. Please contact support.' 
    ...
    if (!serviceRoleKey) {
      console.error('[Orders API] ❌ SUPABASE_SERVICE_ROLE_KEY is not set!')
      console.error('[Orders API] Form submissions will fail due to RLS policies.')
      console.error('[Orders API] Please add SUPABASE_SERVICE_ROLE_KEY to your Vercel environment variables.')
      return NextResponse.json(
        { 
          success: false, 
          error: 'Server configuration error. Please contact support.',
    ...
    if (!isSupabaseAdminConfigured()) {
      console.error('[Orders API] ❌ Supabase admin client is not properly configured!')
      return NextResponse.json(
        { 
          success: false, 
          error: 'Server configuration error. Please contact support.',
